...
static int wait_for_frame(int fd, int timeout_ms)
{
	while(1)
	{
		fd_set read_fds;
...
int v4l2_capture_init(V4L2Capture *cap, const char *device, int width, int height)
{
...

	{
		...
		{
			printf("device does not support multi-planar video capture\n");
			errno = ENOTSUP;
			goto FAIL;
		}
	...
	printf("capture format: %dx%d fourcc=%s planes=%d "
	...
	{
		printf("camera did not accept NV12 format\n");
		errno = ENOTSUP;
		goto FAIL;
	}

	/* 
	 * The currently verified /dev/video11 path reports one V4L2 plane that
	 * contains both Y and interleaved UV data. Reject other layouts explicitly
	 * instead of silently treating them as one contiguous NV12 image.
	 * */
	if (cap->nplanes != 1)
	{
		printf("current encoder path requires one contiguous NV12 plane, "
				"driver returned %d planes\n",
				cap->nplanes);
		errno = ENOTSUP;
		goto FAIL;
	}
	...
	{
		printf("warning: VIDIOC_S_PARM 25 fps failed: %s\n", strerror(errno));
		cap->fps_numerator = 1;
		cap->fps_denominator = 0;
	}
	else
	{
	...
	if (cap->fps_numerator != 0)
	{
		printf("capture frame interval: %u/%u s (%.2f fps)\n",
				cap->fps_numerator,
				cap->fps_denominator,
				(double)cap->fps_denominator / (double)cap->fps_numerator);
	}
	...
	if (request.count < 2 || request.count > V4L2_CAPTURE_MAX_BUFFERS)
	{
		printf("unexpected V4L2 buffer count: %u\n", request.count);
		errno = ENOMEM;
		goto FAIL;
	}
	...
	for(i=0; i<cap->buffer_count; ++i)
	{
		...
		{
			perror("VIDIOC_QBUF during init");
			goto FAIL;
		}

		printf("capture buffer %u: addr=%p length=%zu\n",
				i,
				cap->buffers[i].addr,
				cap->buffers[i].length);
	}
...
int v4l2_capture_acquire_frame(V4L2Capture *cap, V4L2Frame *frame, int timeout_ms)
{
	...
	{
		printf("driver returned invalid buffer index %u\n", buffer.index);
		errno = EIO;
		return -1;
	}
			...
			bytes_used > cap->buffers[buffer.index].length)
	{
		...
		printf("invalid captured plane metadata: index=%u "
				"bytesused=%zu data_offset=%zu mapped=%zu\n",
				buffer.index,
				bytes_used,
				data_offset,
	...
	frame->timestamp_us = (int64_t)buffer.timestamp.tv_sec * 1000000LL + (int64_t)buffer.timestamp.tv_usec;
	frame->valid = 1;
...
int v4l2_capture_get_frame(V4L2Capture *cap, uint8_t **data, int *size)
{
	...
	if (frame.size > cap->legacy_copy_capacity)
	{
		...
		if (new_copy == NULL)
		{
			printf("realloc legacy frame copy failed, size=%zu\n", frame.size);
			v4l2_capture_release_frame(cap, &frame);
			return -1;
		}
...
void v4l2_capture_close(V4L2Capture *cap)
{
	...
	for (i = 0; i < cap->buffer_count; ++i)
	{
		if (cap->buffers[i].addr != NULL && cap->buffers[i].length > 0)
		{
