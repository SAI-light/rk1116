 ...
 *      ChangeLog:  1, Release initial version on "07/28/2026 11:02:35 PM"
 *                 
 ********************************************************************************/
...
int main(int argc, char *argv[])
{
	...
	const size_t expected_nv12_size = (size_t)VIDEO_WIDTH * (size_t)VIDEO_HEIGHT * 3U / 2U;
	...
	if (argc >= 4 && parse_positive_int(argv[3], &requested_frames) != 0)
	{
	...
	if (v4l2_capture_init(&capture, device, VIDEO_WIDTH, VIDEO_HEIGHT) < 0)
	{
	...
	if (capture.width != VIDEO_WIDTH || capture.height != VIDEO_HEIGHT)
	{
	...
	{
		if (v4l2_capture_acquire_frame(&capture, &frame, CAPTURE_TIMEOUT_MS) < 0)
		{
	...
	if (mpp_encoder_init(&encoder, VIDEO_WIDTH, VIDEO_HEIGHT) < 0)
	{
	...
	if (mp4_muxer_init(&muxer, MP4_MUXER_ID, output_path, VIDEO_WIDTH, VIDEO_HEIGHT, VIDEO_FRAME_RATE, VIDEO_BIT_RATE) < 0)
	{
	...
	while (encoded_frames < requested_frames && !g_stop)
	{
		...
		if (have_previous_sequence && captured_sequence > previous_sequence + 1U)
		{
			dropped_capture_frames += (uint64_t)(captured_sequence - previous_sequence - 1U);
		}
		...
		h264_len = mpp_encoder_encode(&encoder, frame.data, (int)expected_nv12_size, &h264_data);
		...
		pts_us = (int64_t)encoded_frames * 1000000LL / VIDEO_FRAME_RATE;

		mux_start_us = get_time_us();

		if (mp4_muxer_write_h264(&muxer, h264_data, (size_t)h264_len, pts_us, &key_frame) < 0)
		{
...
CLEANUP:
	...
	printf("encoded frames      : %d/%d\n", encoded_frames, requested_frames); 
	printf("key frames          : %d\n", key_frames); 
	...
	if (encoded_frames > 0)
	{
		printf("average acquire wait: %.3f ms/frame\n", (double)total_acquire_us / (double)encoded_frames / 1000.0); 

		printf("average encode time : %.3f ms/frame\n", (double)total_encode_us / (double)encoded_frames / 1000.0); 

		printf("average mux time    : %.3f ms/frame\n", (double)total_mux_us / (double)encoded_frames / 1000.0); 

		printf("average packet size : %.2f bytes/frame\n", (double)total_h264_bytes / (double)encoded_frames); 

		printf("timeline duration   : %.3f s\n", (double)encoded_frames / (double)VIDEO_FRAME_RATE);
	}

	if (total_encode_us > 0 && encoded_frames > 0)
	{
		printf("encode-only speed   : %.2f fps\n", (double)encoded_frames * 1000000.0 / (double)total_encode_us);
	}

	if (wall_start_us >= 0 && wall_end_us > wall_start_us && encoded_frames > 0)
	{
		int64_t wall_us = wall_end_us - wall_start_us;

		printf("wall-clock time     : %.3f s\n", (double)wall_us / 1000000.0);

		printf("end-to-end speed    : %.2f fps\n", (double)encoded_frames * 1000000.0 / (double)wall_us);
	}
